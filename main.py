...
def train_test_model(args):
    ...
    model = MSDRP(2210, 2698,args.embed_dim, args.batch_size, args.droprate, args.droprate).to(args.device)
    if args.mode == 'train':
        Regression_criterion = nn.MSELoss()
        optimizer = torch.optim.Adam(model.parameters(), lr=args.lr, weight_decay=args.weight_decay)

        log_folder = os.path.join(os.getcwd(), "result/log_best", model._get_name())
